...
def suggest_moisturizer_interaction(moisturizer_name, thickener_name):
    ...
    return pairs.get(key) or pairs.get(reverse_key) or "해당 조합에 대한 상호작용 정보가 없습니다."

def analyze_moisturizer_warning(water_ratio, moisturizer_ratio):
    if water_ratio > 70 and moisturizer_ratio < 3:
        return "⚠️ 보습제가 부족할 수 있습니다. 수상 비율이 높을 경우 보습제를 3% 이상으로 설정하는 것을 권장합니다."
    return ""

def check_preservative_safety(preservatives):
    warnings = []
    for p in preservatives:
        name = p["name"].lower()
        percent = p["percent"]
        if "페녹시에탄올" in name and percent > 1.0:
            warnings.append(f"⚠️ {p['name']} 사용량이 1.0%를 초과했습니다. 일반적으로 0.3~1.0%가 적정입니다.")
        elif "파라벤" in name and percent > 0.8:
            warnings.append(f"⚠️ {p['name']} 사용량이 0.8%를 초과했습니다. EU 기준 확인 요망.")
    return warnings

def analyze_misc_type(ingredient_rows):
    found = []
    for ing in ingredient_rows:
        name = ing["name"].lower()
        if "글리세린" in name or "glycol" in name:
            found.append(f"🔍 '{ing['name']}'은(는) 보습제로 사용될 수 있습니다.")
        if "에탄올" in name or "페녹시" in name:
            found.append(f"🔍 '{ing['name']}'은(는) 방부 효과가 있을 수 있습니다.")
    return found
...
if __name__ == "__main__":
    ...
    for thick in thickeners:
        print(f"- {thick['name']} ({thick['range']}): {thick['note']}")

    print("\n[보습제-점증제 상호작용]")
    print(suggest_moisturizer_interaction("글리세린", "카보머"))

    print("\n[보습제 경고 분석]")
    print(analyze_moisturizer_warning(water_ratio=75, moisturizer_ratio=2))

    print("\n[보존제 안전성 체크]")
    preservatives = [
        {"name": "페녹시에탄올", "percent": 1.2},
        {"name": "메틸파라벤", "percent": 0.9}
    ]
    for msg in check_preservative_safety(preservatives):
        print(msg)
    ...
    for msg in analyze_misc_type(misc):
        print(msg)
