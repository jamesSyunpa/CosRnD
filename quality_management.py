...
class QualityManagementFrame(ctk.CTkFrame):
    ...
    def setup_semi_finished_product_tab(self, tab_frame):
        ...
        # --- UI 재구성: 실제 문서 양식처럼 보이도록 변경 ---
        report_frame = ctk.CTkScrollableFrame(tab_frame, label_text=self.texts['semi_finished_product_report_title'])
        ...
        def create_cell(parent, row, col, text, rowspan=1, colspan=1, is_header=False, is_input=False, key=None):
            """문서 양식의 셀을 생성하는 헬퍼 함수"""
            frame = ctk.CTkFrame(parent, corner_radius=0, border_width=1, border_color=("gray70", "gray40"))
            frame.grid(row=row, column=col, rowspan=rowspan, columnspan=colspan, sticky="nsew")
            
            if is_input and key:
                entry = ctk.CTkEntry(frame, corner_radius=0, border_width=0, fg_color="transparent")
                entry.pack(fill="both", expand=True, padx=5, pady=2)
                self.semi_product_entries[key] = entry
                return entry
            else:
                label = ctk.CTkLabel(frame, text=text, font=ctk.CTkFont(weight="bold" if is_header else "normal"))
