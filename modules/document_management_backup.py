...
class DocumentManagementFrame(ctk.CTkFrame):
    ...
    def search_ingredients_by_list(self):
        ...
        search_terms = [line.strip() for line in input_text.split('\n') if line.strip()]
    ...
    def setup_production_tab(self, tab_frame):
        ...
        ctk.CTkButton(runs_toolbar, text="삭제", width=55, command=self.delete_selected_production_run, font=("", 11)).pack(side="left")
    ...
    def create_production_formulation(self):
        ...
        try:
            ...
            try:
                ...
                # fallback: no command support, try after rendering
                pass
            # initial populate
            try:
...
비고: {prod_snapshot.get('비고', '')}"""
    ...
    def _create_client_folder_card(self, parent, client_id, client_name, item_count):
        ...
        # 업체명
        name_label = ctk.CTkLabel(card, text=client_name, font=ctk.CTkFont(weight="bold"),
                                 wraplength=120)
        name_label.pack()
        name_label.bind("<Button-1>", lambda e, cid=client_id, cname=client_name: self._on_client_folder_click(cid, cname))
        
        # 아이템 수
        count_label = ctk.CTkLabel(card, text=f"({item_count}개 아이템)", font=ctk.CTkFont(size=11),
                                  text_color="gray50")
