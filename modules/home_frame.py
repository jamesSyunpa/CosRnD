...
class HomeFrame(ctk.CTkFrame):
    ...
    def _load_system_updates(self):
        ...
        # 공식 릴리즈 패치노트 비동기 로드 (순수 시스템 업데이트 내역 전용)
        def _fetch_system_updates():
            try:
                ...
                print(f"[Home] 시스템 업데이트 내역 로드 오류: {e}")

        threading.Thread(target=_fetch_system_updates, daemon=True).start()
