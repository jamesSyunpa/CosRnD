...
from database.models import Formulation, FormulationItem, Material, Client, User
...
class SettingsManagementFrame(ctk.CTkFrame):
    ...
    def browse_db_path(self):
        path = filedialog.asksaveasfilename(
            ...
            initialfile="cosmetic_shared.db",
            defaultextension=".db",
    ...
    def save_paths(self):
        ...
        db_path_changed = new_db_path != old_db_path
        ...
        if self.current_user.is_admin and db_path_changed:
            ...
            if handler:
                handler(new_db_path, new_excel_path)
        
        elif db_path_changed: # Non-admin user
            self._save_and_restart(new_db_path, new_excel_path, "DB 경로가 변경되었습니다. 프로그램을 재시작합니다.")
    ...
    def _save_and_restart(self, new_db_path, new_excel_path, message):
        self._save_config('Paths', 'shared_db_path', new_db_path)
    ...
    def _handle_create_new_db(self, new_db_path, new_excel_path):
        if os.path.exists(new_db_path):
            if not messagebox.askyesno("경고", f"파일이 이미 존재합니다: '{os.path.basename(new_db_path)}'\n기존 파일을 덮어쓰고 비어있는 DB를 새로 만드시겠습니까?", parent=self):
                return
        self._save_and_restart(new_db_path, new_excel_path, "비어있는 새 공유 DB 경로가 설정되었습니다. 프로그램을 재시작합니다.")
    ...
    def _handle_move_db(self, new_db_path, new_excel_path):
        ...
        try:
            ...
            self._save_and_restart(new_db_path, new_excel_path, "기존 DB를 새 공유 경로로 이동했습니다. 프로그램을 재시작합니다.")
        except Exception as e:
            messagebox.showerror("오류", f"DB 파일 이동 중 오류가 발생했습니다: {e}", parent=self)
            self.app.after(100, lambda: db_manager.setup_database(self.application_path, self.config_path, self.app.on_initial_setup))

    def _handle_use_existing_db(self, new_db_path, new_excel_path):
        if not os.path.exists(new_db_path):
            messagebox.showerror("오류", f"선택한 경로에 DB 파일이 존재하지 않습니다.\n'{new_db_path}'", parent=self)
            return
        self._save_and_restart(new_db_path, new_excel_path, "새로운 공유 DB 경로가 설정되었습니다. 프로그램을 재시작합니다.")
